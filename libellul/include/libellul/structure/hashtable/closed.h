 ...
 *
 * T_MAP_TAG: Prefix for generated type and function names
 * T_MAP_ENTRY: The entry type to store in the hash table
 * T_MAP_KEY: The key type within the entry
 * 
 * Required user-defined functions:
 * 
 * size_t T_MAP_TAG_hash(const T_MAP_KEY *key)
 * int T_MAP_TAG_eq(const T_MAP_KEY *a, const T_MAP_KEY *b)
 *
 * Optional parameters:
 *
 * T_MAP_NBUCKETS: Initial number of buckets (default: 16)
 *
 * Example usage:
 *
 * typedef struct {
...
// #if !defined(T_MAP_EXPORT_DEFS)
// T_MAP_INTERFACE int CLOSED_METHOD(is_empty)(T *ht) {
