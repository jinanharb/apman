...
/* ============================================
   Benchmark for Linear Find
   ============================================ */
double benchmark_linear_find(size_t N) {
    // Pre-populate
    ...
    for(int run = 0; run < RUNS; run++) {
        for (size_t key = 0; key < N; key++) {
            int found = dictlinear_contains(map, (int)key);
            assert(found);
        }
    }
...
/* ============================================
   Benchmark for Closed Find
   ============================================ */
double benchmark_closed_find(size_t N) {
    // Pre-populate
    ...
    for(int run = 0; run < RUNS; run++) {
        for(size_t key = 0; key < N; key++) {
            int k = (int)key;
            closed_entry_t *found = closed_map_find(&map, &k);
            assert(found != NULL);
        }
    }
    uint64_t t1 = get_time_nsec();
...
int main(void) {
    ...
    for(size_t logN = LOG2_N_MIN; logN <= LOG2_N_MAX; logN++) {
        ...
        double t_linear = benchmark_linear_find(N);
        double t_closed = benchmark_closed_find(N);
